...
from ai.acquisition.consumer.parser import extract_text_from_pdf, extract_text_from_html, infer_information_type
from dataclasses import asdict

OUT_DIR = Path("data/catalog/phase11_2c_consumer")
RAW_DIR = Path("data/raw/immutable/consumer")
...
def acquire(pilot=False):
    ...
    # Prioritize publicly accessible BIS consumer material FIRST
    start_urls = [
        "https://www.bis.gov.in/index.php/consumer-affairs/",
        "https://www.bis.gov.in/index.php/consumer-affairs/complaints/",
        "https://www.bis.gov.in/index.php/consumer-affairs/bis-care-app/",
        "https://www.bis.gov.in/index.php/consumer-affairs/consumer-awareness/",
        "https://www.manakonline.in/MANAK/Complaints"  # Operational portal
    ]
    
    discovery = ConsumerDiscovery(start_urls)
    records = []
    failures = []
    seen_urls = set()
    ...
    while to_visit:
        ...
        if source_type == "PDF":
            ...
            for link in new_links:
                # Basic filter: only crawl consumer/complaint related links to prevent inflating domain
                # with irrelevant BIS standards data.
                link_lower = link.lower()
                if "consumer" in link_lower or "complaint" in link_lower or "care" in link_lower:
                    if link not in seen_urls and link not in to_visit:
                        # Append operational portals to end, prioritize bis.gov.in
                        if "manakonline" in link:
        ...
        rec = ConsumerRecord(
            record_id=str(uuid.uuid4()),
            record_type="CONSUMER_DOCUMENT",
            title=f"Consumer Source: {url.split('/')[-1] or 'overview'}",
    ...
    write_jsonl(OUT_DIR / "consumer_records.jsonl", records)
